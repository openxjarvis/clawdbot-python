...
- **Still programmable**: Each step can call any CLI or script. If you want Python, generate `.lobster` files from code.
...
Use `approve --preview-from-stdin --limit N` to attach a JSON preview to approval requests without custom Python/shell glue. Resume tokens are now compact: Lobster stores workflow resume state under its state dir and hands back a small token key.
