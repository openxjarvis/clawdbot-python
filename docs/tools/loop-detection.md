...
- **`openclaw/agents/tool_loop_detection.py`** — `detect_tool_call_loop()`, `record_tool_call()`, `record_tool_call_outcome()`, `hash_tool_call()`, `hash_tool_outcome()`, `_is_known_poll_tool_call()`.

### Known poll tools (aligned with TS)

The `known_poll_no_progress` detector triggers for:
- `command_status` (any params)
- `process` with `action="poll"` or `action="log"`

These tools use a result-hash-aware no-progress streak, not a simple repeat count.

### Detector ordering (mirrors TS `detectToolCallLoop`)

1. `known_poll_no_progress` — polling tools checked first
2. `ping_pong` — alternating tool pair with stable outcomes
3. `generic_repeat` — non-poll tools only, warning-level
4. `global_circuit_breaker` — absolute cap on history size
