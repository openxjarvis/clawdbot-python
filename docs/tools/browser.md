...
Playwright package and restart the gateway, or reinstall OpenClaw with browser support.

#### Docker Playwright install

If your Gateway runs in Docker, use the bundled CLI:
...
- Download and trace output paths are constrained to OpenClaw temp roots:
  - traces: `/tmp/openclaw` (fallback: `tempfile.gettempdir()/openclaw`)
  - downloads: `/tmp/openclaw/downloads` (fallback: `tempfile.gettempdir()/openclaw/downloads`)
- Upload paths are constrained to an OpenClaw temp uploads root:
  - uploads: `/tmp/openclaw/uploads` (fallback: `tempfile.gettempdir()/openclaw/uploads`)
- `snapshot`:
