...
Provider keys accept either `provider` (e.g. `google`) or
...
- Session targeting is controlled by `tools.sessions.visibility` (default `tree`: current session + spawned subagent sessions).
