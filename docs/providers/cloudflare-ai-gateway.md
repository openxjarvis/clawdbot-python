...
read_when:
  - You want to use Cloudflare AI Gateway with OpenClaw Python
...
If the Gateway runs as a daemon (systemd or similar), make sure `CLOUDFLARE_AI_GATEWAY_API_KEY` is available to that process (for example, in `~/.openclaw/.env` or via `env.shellEnv`).
