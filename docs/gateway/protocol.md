...
- `PROTOCOL_VERSION` lives in `openclaw/gateway/protocol/schema.py`.
- Clients send `minProtocol` + `maxProtocol`; the server rejects mismatches.
- Protocol schema is defined using Python dataclasses/Pydantic models.
...
Pydantic schemas in `openclaw/gateway/protocol/schema.py`.
