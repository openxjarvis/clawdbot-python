...
1. **Parse once:** use the shared `markdown_to_ir(...)` helper with channel-appropriate
   options (autolink, heading style, blockquote prefix).
2. **Render:** implement a renderer with `render_markdown_with_markers(...)` and a
   style marker map (or Signal style ranges).
3. **Chunk:** call `chunk_markdown_ir(...)` before rendering; render each chunk.
