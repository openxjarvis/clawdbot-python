...
- `kinds?: list[str]` filter: any of `"main" | "group" | "cron" | "hook" | "node" | "other"`
- `limit?: int` max rows (default: server default, clamp e.g. 200)
- `activeMinutes?: int` only sessions updated within N minutes
- `messageLimit?: int` 0 = no messages (default 0); >0 = include last N messages
...
- `limit?: int` max messages (server clamps)
- `includeTools?: bool` (default False)

Behavior:

- `includeTools=False` filters `role: "toolResult"` messages.
- Returns messages list in the raw transcript format.
...
- `sessionKey` (required; accepts session key or `sessionId` from `sessions_list`)
- `message` (required)
- `timeoutSeconds?: int` (default >0; 0 = fire-and-forget)
