...
| Bot 卡住 / 复杂任务后无响应 | Agent 任务陷入循环或超时 | 在 Telegram 发送 `/stop`；任务会在 3 分钟后自动超时 |
| 消息排队但始终不处理 | 上一个任务仍在运行 | 发送 `/stop` 或等待 3 分钟自动超时 |
...
在**私信**中，Bot 每次调用工具之前都会单独发送一条消息，展示当前正在做什么，让你实时跟进执行进度。在**群组**中，Bot 会显示一个实时更新的预览气泡，最终消息出现后无缝替换。
...
|------|------|
| `/reset` | 开启新会话 |
| `/stop` | 中止当前正在运行的任务并清空消息队列 |
| `/queue <模式>` | 更改消息队列行为：`interrupt`（新消息取消当前任务）、`steer`（中途注入）、`followup`（排队等待，默认）、`collect` |
| `/cron` | 查看定时任务 |
| `/help` | 显示帮助 |

> **何时用 `/stop`：** 当 Bot 对复杂任务卡住或长时间没有反应时，发送 `/stop` 可立即终止当前任务，让下一条消息重新开始。Agent 任务也会在 **3 分钟**后自动超时。
...
无需手动编辑 JSON，使用内置的权限档位系统，一条命令切换四个预设级别。

每个档位覆盖**三个权限维度**——执行权限、入站权限和出站权限：

| # | 档位 | exec.security | dmPolicy（入站） | groupPolicy（入站） | allowWithinProvider（出站） | allowAcrossProviders（出站） |
|---|---|---|---|---|---|---|
| 1 | **Relaxed（宽松）** | `full` | `open` | `open` | `true` | `true` |
| 2 | **Trusted（信任）** ← **推荐** | `full` | `pairing` | `allowlist` | `true` | `false` |
| 3 | **Standard（标准）** | `allowlist` | `pairing` | `allowlist` | `true` | `false` |
| 4 | **Strict（严格）** | `deny` | `pairing` | `disabled` | `false` | `false` |

**为什么推荐 Trusted 档位？** 全量 Agent 能力 + 配对机制，可玩性强的同时需要用户审批才能接入。群组聊天仅限白名单（非公开），Agent 可在同一频道内跨会话发消息，但不能跨频道服务商发送。

**Standard 档位默认 safe_bins：** `python`、`pip`、`uv`、`ffmpeg`、`git`、`node`、`npm`、`convert`
...
> 档位会同时设置 `exec.security`、`safe_bins`、所有已配置频道的 `dmPolicy` 与 `groupPolicy`，以及 `tools.message.crossContext` 出站配置。
> 应用后重启 OpenClaw：`uv run openclaw start`
...
| `message.crossContext.allowWithinProvider` | 允许 Agent 向同频道内的其他对话主动发消息（如 Telegram → 其他 Telegram 群/用户）| `true`（默认）/ `false` |
| `message.crossContext.allowAcrossProviders` | 允许 Agent 跨频道发消息（如在 Telegram 会话中向 Discord 发消息）| `false`（默认）/ `true` |

> **关键：** `exec.security: "deny"` 是最常见的"Agent 无法生成文件"的原因。如需 Agent 运行脚本（生成 PPT、视频、音频等），改为 `full` 或 `allowlist`。

> **快捷方式：** 使用 `uv run openclaw security preset` 可以一次性切换所有权限设置（包括入站与出站），无需手动编辑 JSON。详见[权限档位](#5-权限档位--一键切换)。
...
| `dmScope` 值 | 效果 |
|---|---|
| `per-channel-peer` **（默认）** | 每个 `(频道, 用户)` 组合拥有独立会话，符合聊天机器人的自然预期 |
| `per-peer` | 按用户 ID 隔离，同一用户在 Telegram 和飞书共享同一会话 |
| `per-account-channel-peer` | 按 `(账号, 频道, 用户)` 三维隔离，适合同时运行多个 Bot 账号 |
| `main` | 所有频道的所有私信共享一个会话（不推荐作为默认值）|

> **说明：** 默认 `per-channel-peer` 意味着你的 Telegram 对话和飞书对话完全独立、互不干扰。
> 如果希望 Agent 跨 Telegram 和飞书记住同一个用户的上下文，改为 `per-peer`。
...
| 命令 | 说明 |
|------|------|
| `status` | 实时概览：Gateway 延迟、频道状态（OK/WARN/OFF）、会话数、运行时长 |
| `status --json` | 同上，输出为机器可读 JSON（gateway、agent、channels、sessions）|
| `status --all` | 完整诊断：配置路径、agent 列表、gateway 日志尾部、频道详情 |
| `status --deep` | 主动探测所有频道（每个频道实时健康检查）|
| `status health` | 快速可达性检查：gateway 运行时长、连接数、延迟 |
| `status sessions` | 列出所有已存储的会话，显示年龄、类型、模型 |
