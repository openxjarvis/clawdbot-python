...
class CronAddParams(BaseModel):
    """
    Parameters for cron.add — accepts flat job fields directly (TypeScript frontend sends them flat,
    not nested under a 'job' key).
    ...
    schedule: dict = Field(..., description="Schedule configuration with 'kind' field")
    payload: dict = Field(..., description="Payload configuration with 'kind' field")
...
class CronRemoveParams(BaseModel):
    """Parameters for cron.remove — accepts 'id' (frontend) or 'jobId' (legacy)."""
    id: Optional[str] = None
    jobId: Optional[str] = None

    @model_validator(mode="after")
    def require_id(self) -> "CronRemoveParams":
        if not self.id and not self.jobId:
